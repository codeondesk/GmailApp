...
fun GmailDrawerMenu(scrollState: ScrollState){
    ...
    Column(Modifier.verticalScroll(scrollState)){
        ...
        menuList.forEach {
            item ->
            when {
                item.isDivider -> {  Divider(modifier = Modifier.padding(bottom = 20.dp,top=20.dp))  }
                item.isHeader -> {  Text(text = item.title!!, fontWeight = FontWeight.Light, modifier = Modifier.padding(start = 20.dp, bottom = 20.dp, top=20.dp))  }
